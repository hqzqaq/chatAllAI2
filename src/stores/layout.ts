...
export const useLayoutStore = defineStore('layout', () => {
    ...
    const cardWidth = computed(() =>
        (availableWidth.value - gridSettings.value.gap * (gridSettings.value.columns - 1)) / gridSettings.value.columns
    )

    const cardHeight = computed(() =>
        (availableHeight.value - gridSettings.value.gap * (gridSettings.value.rows - 1)) / gridSettings.value.rows
    )
     ...
     * 获取卡片配置
     */
    const getCardConfig = (providerId: string): CardConfig | undefined => {
        return cardConfigs.value[providerId]
    }
