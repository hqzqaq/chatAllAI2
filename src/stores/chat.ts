...
export const useChatStore = defineStore('chat', () => {
    ...
    const loggedInProviders = computed(() =>
        providers.value.filter(provider => provider.isLoggedIn)
    )
    ...
    const getSendingStatus = (providerId: string): 'idle' | 'sending' | 'sent' | 'error' => {
        return sendingStatus.value[providerId] || 'idle'
    }

    /**
     * 检查是否有正在发送的消息
     */
    const hasSendingMessages = (): boolean => {
        return Object.values(sendingStatus.value).some(status => status === 'sending')
    }
     ...
     * 获取对话历史
     */
    const getConversation = (providerId: string): Message[] => {
        return conversations.value[providerId] || []
    }
