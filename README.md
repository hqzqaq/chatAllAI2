...
├── src/               # Vue3渲染进程
│   ├── components/    # 组件层
│   │   ├── chat/      # 聊天相关组件 (AICard, UnifiedInput, SummaryCard)
│   │   ├── webview/   # WebView封装组件 (WebView)
│   │   ├── layout/    # 布局组件 (AppLayout, AppHeader, AppFooter)
│   │   ├── summary/   # 总结侧边栏组件
│   │   └── settings/  # 设置相关组件
│   ├── composables/   # 组合函数 (useLoginCheck, useSessionPersistence, useWebViewEvents, useSummary)
│   ├── config/        # 配置层 (providers.ts — AI提供商配置)
│   ├── stores/        # 状态管理 (Pinia)
│   ├── services/      # 服务层 (MessageDispatcher, SummaryService)
│   ├── types/         # 类型定义
│   └── utils/         # 工具函数 (脚本解析、存储封装、WebView辅助)
├── public/           # 静态资源 & AI提供商图标
└── scripts/          # 构建脚本
```
...
2. **electron/managers/IPCHandler.ts**：IPC 通信统一处理，通过 `executeInWebViewContainer` 公共方法向 WebView 注入脚本
3. **src/App.vue**：应用根组件，设置全局样式和主题
4. **src/views/Chat.vue**：核心聊天界面，管理 AI 卡片布局，总结逻辑已抽离到 `useSummary` composable
5. **src/config/providers.ts**：AI 提供商配置数据层，通过 `createDefaultProvider()` 工厂函数统一创建
6. **src/composables/useLoginCheck.ts**：登录状态检测组合函数
7. **src/composables/useSessionPersistence.ts**：会话保存/恢复组合函数
8. **src/composables/useWebViewEvents.ts**：WebView 事件绑定组合函数（回调驱动）
9. **src/composables/useSummary.ts**：AI 总结流程组合函数
10. **src/utils/storage.ts**：统一 localStorage 操作封装（类型安全）
11. **src/utils/webviewHelper.ts**：WebView ID 构建/解析工具函数
